...
new_bioprosthetics <- bioprosthetic_group[1:664,]
new_mechprosthetics <- mechprosthetic_group[results$proposals[1:664], ]
...
Adding a column for death based on commordities

```{r}
genDeath <- function(data){
  has_cancer <- 2
  has_congestive_heart_failure <- 1
  has_emergency <- 1
  has_age_greater_60 <- 2
  ...
  data$death <- ifelse(score>=3, 1, 0)
  return(data$death)
}
death_mechprosthetics <- new_mechprosthetics
death_mechprosthetics$death <- genDeath(death_mechprosthetics)
#length(which(death_mechprosthetics$death == 1)) # 279

death_bioprosthetics <- new_bioprosthetics
death_bioprosthetics$death <- genDeath(death_bioprosthetics)
#length(which(death_bioprosthetics$death == 1)) # 279
...
Now, we will go ahead and group these people in one single data.frame to apply survival analysis.
...
```{r}
survdiff(Surv(death) ~ group, data = grouped_data)
#survival_model <- survfit(Surv(death)~1, data = grouped_data)
#summary(survival_model)
#plot(survival_model, xlab="Deaths", ylab="Survival Probability")

survival_model_groups <- survfit(Surv(death)~group, data = grouped_data)
summary(survival_model_groups)

survdiff(Surv(death) ~ year, data = grouped_data)
survival_model_years <- survfit(Surv(death)~year, data = grouped_data)
summary(survival_model_years)

survdiff(Surv(death) ~ year+group, data = grouped_data)
survival_model <- survfit(Surv(death)~year+group, data = grouped_data)
summary(survival_model)
```

```{r}
#create survival object
grouped_data$SurvObj <- with(grouped_data, Surv(death))
head(grouped_data)

# Cox regression using coxph
coxph(SurvObj ~age + emergent_admission +        
        concomitant_tricuspid_valve_repair + coagulation + hypertension +
        diabetes_mellitus + sepsis + coronary_artery_disease +
        congestive_heart_failure + atrial_fibrillation + 
        peripheral_vascular_disease + cerebro_vascular_disease +
        chronic_obstructive_pulmonary_disease + chronic_kidney_disease +
                               liver_disease + cancer + group, data =  grouped_data)
