...
Let us find the number of times water content in food products when water content is more than the median water content in foods. Finding the top 2 categories of food
```{r}
variation<- sqldf("SELECT category, SUM(CASE when Water>(SELECT median(Water) FROM fcode_nut_catg) then 1 else 0 end) water 
            FROM fcode_nut_catg
            GROUP BY category
            ORDER BY water DESC
            LIMIT 2")
print(variation)
```
It can bee seen that vegetables followed by fruits give the most water content in everydy life.

Let us find the number of times cholesterol content in food products when cholesterol content is more than the median cholesterol content in foods. Finding the top 2 categories of food
```{r}
variation<- sqldf("SELECT category, SUM(CASE when Cholesterol>(SELECT median(Cholesterol) FROM fcode_nut_catg) then 1 else 0 end) cholesterol 
            FROM fcode_nut_catg
            GROUP BY category
...
print(variation)
```
It can be seen that meat,fish followed by grains supply more than the median level of cholestrol in any foods

Let us find the number of times Iron content in food products when Iron content is more than the median Iron content in foods. Finding the top 2 categories of food
```{r}
variation<- sqldf("SELECT category, SUM(CASE when Iron>(SELECT median(Iron) FROM fcode_nut_catg) then 1 else 0 end) iron 
            FROM fcode_nut_catg
            GROUP BY category
            ORDER BY iron DESC
            LIMIT 2")
print(variation)
```
It can be seen that vegetables, closely followed by grains supply more than the median level of Iron in any foods

Further, the dataset modified before the above analysis has a lot of unwanted nutrients which do not have a clear identification. As can be seen below:
...
Selecting food_codes and descriptions for foods where the description starts with milk.
```{r}
milk_desc <- sqldf("SELECT food_code, main_food_description
                   FROM MainFoodDesc 
                   WHERE main_food_description LIKE 'Milk%'")
print(head(milk_desc))
```

Selecting all nutrients which have some form of vitamin in them
```{r}
vitamin_desc <- sqldf("SELECT nutrient_code, nutrient_description
                   FROM NutDesc 
                   WHERE nutrient_description LIKE '%Vitamin%'")
print(head(vitamin_desc))

count_nuts <- sqldf("SELECT count(DISTINCT(nutrient_code)) FROM vitamin_desc")
print(count_nuts)
```
There are 9 different types of vitamins in our data frame.


Selecting all nutrients which have "a" as the second alphabet in the code
```{r}
a_desc <- sqldf("SELECT nutrient_code, nutrient_description
                   FROM NutDesc 
                   WHERE nutrient_description LIKE '_a%'")
print(head(a_desc))

count_a <- sqldf("SELECT count(DISTINCT(nutrient_code)) FROM a_desc")
print(count_a)
```

There are 10 nutrients with "a"" as the second alphabet.
...
g_res41_nooutlier + geom_boxplot(aes(x=food_category, y=value, fill=nutrient)) +
   theme(legend.position="bottom", legend.direction="horizontal", 
        axis.text.x = element_text(angle = 45, hjust=1)) + ylab("Nutrient Value") +
...
ggplot(df5, aes(x = value)) + geom_density(aes(color=variable)) +
  theme(legend.position="bottom", legend.direction="horizontal", 
        axis.text.x = element_text(angle = 45, hjust=1))+ ylab("Nutrient Density") +
...
ggplot(df5, aes(x = value)) + geom_density(aes(color=variable)) +
  theme(legend.position="bottom", legend.direction="horizontal", 
        axis.text.x = element_text(angle = 45, hjust=1)) + xlim(0,50)
...
ggplot(df6, aes(x = value)) + geom_density(aes(color=variable)) +
  theme(legend.position="bottom", legend.direction="horizontal", 
        axis.text.x = element_text(angle = 45, hjust=1)) + xlim(0,10)+ 
...
g1 + geom_boxplot(aes(x = variable, y = value, fill = Category))+
    ...
    labs(x="Nutrient",y="Nutrient Value ") + ylim(0,700)+ theme(legend.position="bottom", legend.direction="horizontal",  axis.text.x = element_text(angle = 45, hjust=1))
```
...
<h2> Part-5: Final Data Plotting</h2>

```{r}
g <- ggplot(data_final,aes(x=Category))
g+geom_bar(aes(fill=Category))+ggtitle("Food Category counts") + theme(legend.position="none")
```
The above figure shows the number of items in every food category. 

```{r}
g + geom_point((aes(x=Iron ,y= Energy, color=Category)))+ggtitle("Iron and Energy Content")+
  xlab("Iron")+ylab("Energy") + xlim(0,10) + ylim(0,300)
```
Iron and Energy show no distinctive realtionship between them. However, for vegetables, increase in Iron does not affect energy supplied by the food.

```{r}
g + geom_point((aes(x=Total_Fat ,y= Energy, color=Category))) + ggtitle("Fat and Energy Content")+
  xlab("Fat")+ylab("Energy") + theme(legend.position="bottom", legend.direction="horizontal", axis.text.x = element_text(angle = 45, hjust=1))
```
Fat and Energy show a slight linear realtionship between them for most categories except for in vegetables, alcohol and dairy. 

```{r}
g + geom_boxplot(aes(x=Category,y=Total_Sugar)) + ggtitle("Distribution of Sugar")+ theme(legend.position="bottom", legend.direction="horizontal", axis.text.x = element_text(angle = 45, hjust=1))
```
Sugars, grains and dairy contain high volumes of sugar

```{r}
g+geom_boxplot(aes(x=Category,y=Water))+ggtitle("Water Content by category")+ theme(legend.position="bottom", legend.direction="horizontal", axis.text.x = element_text(angle = 45, hjust=1))
```
Fruits and vegetables contain a higher content of water when we analyze foods by categories.

```{r}
g + geom_boxplot(aes(x=Category,y=Carbohydrate)) + ggtitle("Carbohydrate Content by category")+ theme(legend.position="bottom", legend.direction="horizontal", axis.text.x = element_text(angle = 45, hjust=1))
```
Sugars and grains contain the highest content of carbohydrate

```{r}
g+geom_boxplot(aes(x=Category,y=Total_Fat))+ggtitle("Fat Content by Category")+ theme(legend.position="bottom", legend.direction="horizontal", axis.text.x = element_text(angle = 45, hjust=1))
```
Fats, legumes, nuts and seeds contain the highest concentration of fats

<h2> Part-6: T-Tests</h2>
...
print(t_test_exp)
```
On the basis of the above results we have a little direction in which e can prform the t-tests
...
p-value = 0.8557: T-test for mean value of Energy in grains and sugars shows that there is not enough evidence to reject the null hypothesis that the means are equal
```{r}
t.test(x = Grains_df$Energy,y = Sugar_df$Energy)
```

<b>IRON</b>
p-value = 0.03503: T-test for mean value of Iron in meat,fish and Vegetables shows that the mean of the two data sets is not the same
```{r}
t.test(x = Meat_df$Iron,y = Veggie_df$Iron)
```

p-value = 0.7429: T-test for mean value of Iron in fruits and dairy shows that there is not enough evidence to reject the null hypothesis that the means are equal
```{r}
t.test(x = Fruits_df$Iron,y = Dairy_df$Iron)
```
...
<b>TOTAL SUGAR</b>
p-value = 9.143e-06: T-test for mean value of Sugar in Vegetables and Fruits shows that the mean of the two data sets is not the same
```{r}
t.test(x = Veggie_df$Total_Sugar,y = Fruits_df$Total_Sugar)
```

p-value = 0.6044: T-test for mean value of Sugar in Dairy and Grains shows that there is not enough evidence to reject the null hypothesis that the means are equal
```{r}
t.test(x = Dairy_df$Total_Sugar,y = Grains_df$Total_Sugar)
```
...
<b>WATER</b>
p-value = 0.6948: T-test for mean value of Water in Meat,fish and Dairy shows that there is not enough evidence to reject the null hypothesis that the means are equal
```{r}
t.test(x = Meat_df$Water,y = Dairy_df$Water)
```

p-value = 0.6976: T-test for mean value of Water in Fats and legumes,nuts,seeds shows that there is not enough evidence to reject the null hypothesis that the means are equal
```{r}
t.test(x = Fats_df$Water,y = Legumes_df$Water)
```

<h2> Part-7: CORRELATIONS Between Nutrients</h2>

```{r}
#scaling data because everything is in different units
dat <- scale(data_final[-ncol(data_final)])

par(mfrow= c(1,1))
corrplot::corrplot(cor(dat, 
                       method = c("pearson", "kendall", "spearman")), tl.cex=0.6, tl.col = "black", title = "Correlations in Nutrients") 
correlations <- cor(dat, use="pairwise")
print(correlations)
```
...
7. This shows that Water and Energy are will almolst never be found together- correlation -0.92
8. This shows that Water and Carbohydrate are will almolst never be found together- correlation -0.77
9. This shows that Iron and Vitamin B12 are highly likely to be found together- correlation 0.80
10. This shows that Phosphorus and Protein are highly likely to be found together- correlation 0.88

__Now, that we have analyzed the dataset as a whole; let us also analyze the correlations within different food categories__

<b>VEGETABLES</b>
```{r}
dat_veg <- scale(Veggie_df[-ncol(Veggie_df)])

corrplot::corrplot(cor(dat_veg), tl.cex=0.6, tl.col = "black", title = "Correlations in Nutrients within Vegetables")
correlations_vegetables <- cor(dat_veg, use="pairwise")
print(correlations_vegetables)
```
1. It can be seen that Phosphorus and Protein are highly likely to be present together in vegetables
2. Sodium and Total Fat are highly likely to be present together in vegetables
3. It can also be seen that Phosphorus and Water are unlikely to be present together in vegetables
4. Similarly, sugar and Calcium are also unlikely to be seen together in vegetables

<b>Please note that broadly, nutrient correlations still show similar correlations as in the bigger dataset.</b>

<b>DAIRY</b>
```{r}
dat_dairy <- scale(Dairy_df[-ncol(Dairy_df)])

corrplot::corrplot(cor(dat_dairy), 
                        tl.cex=0.6, tl.col = "black", title = "Correlations in Nutrients within Dairy products")
correlations_dairy <- cor(dat_dairy , use="pairwise")
print(correlations_dairy)
```
Dairy products show an extreme behaviour. Either they are highly correlated to nutrients or they show strict negative correlations. 
1. It can be seen that Energy and Fat are highly likely to be present together in dairy- correlation 0.97
2. Vitamin B12 and Vitamin D are highly likely to be present together in dairy- correlation 0.95
3. It can also be seen that Vitamin D and Carbohydrate are unlikely to be present together in dairy- correlation -.90
4. Similarly, Water and Enerygy are also unlikely to be seen together in dairy- correlation -0.99

<b>Meat,Fish</b>
```{r}
dat_meat <- scale(Meat_df[-ncol(Meat_df)])

corrplot::corrplot(cor(dat_meat), tl.cex=0.6, tl.col = "black", title = "Correlations in Nutrients within Meat/fish products")
correlations_meat <- cor(dat_meat, use = "pairwise")
print(correlations_meat)
```

Meat,fish products show an behaviour similar to the original correlations of the entire data set.
1. It can be seen that Potassium and Phosphorus are highly likely to be present together in Meat,Fish- correlation 0.84
2. Vitamin B12 and Protein are highly likely to be present together in Meat,Fish- correlation 0.77
3. It can also be seen that Potassium and Carbohydrate are unlikely to be present together in Meat,Fish- correlation -0.71
4. Similarly, Carbohydrate and Protein are also unlikely to be seen together in Meat,Fish- correlation -.62

<h2> Part-8: Support Vector Machines</h2>
...
print("Number of data observations in Fruits and Vegetables data frame are:")
print(observations)
```
...
<b>Splitting the data into train and test sets to apply cross validation as there are only 90 observations in the dataset of vegetables and fruits</b>
We will also be scaling the data because each nutrient is measured on a different scale
```{r}
...
xtrain <- data_x[tindex,]
xtest <- data_x[-tindex,]
ytrain <- data_y[tindex,] 
ytest <- data_y[-tindex,] 
...
```{r}
library(kernlab)
svp <- ksvm(x = xtrain, y = ytrain, type = "C-svc", kernel = 'vanilladot', C = 100, scaled = c()) 
```
...
print(sprintf("Accuracy of the SVM model is: %s percent",accuracy*100))
```

<b> SVM has classified the data into fruits and vegetables with a very good accuracy of 83.78%</b>
...
Changing labels for fruits to 0s and Vegetables to 1s. Consequently, fitting a null model and generating it's summary for model evaluation. If Null Deviance is really small, it means that the Null Model explains the data pretty well.

The residual deviance shows how well the response is predicted by the model when the predictors are included. For example, If the deviance goes up by 3443.3 when 22 predictor variables are added (note: degrees of freedom = no. of observations - no. of predictors). This increase in deviance is evidence of a significant lack of fit.

Also, note that logistic regression does not require the data to be normalized.
...
```
The summary shows Null deviance of 121.91  on 89  degrees of freedom & Residual deviance of 121.91  on 89  degrees of freedom. The degree of freedom has not reduced because this is a null model. The AIC score of 123.91 will be used to analyze the full model.
...
```
This full model gives a Null deviance of 1.2191e+02  on 89  degrees of freedom and Residual deviance of 6.2246e-08  on 75  degrees of freedom. This shows a significant improvement in the model fit. AIC of 30 shows there is very little information loss in this model. But, this model is too complicated.

We will now use the step function to assess the models between a null and a full model which one is the best fit for the data, such that we have the lowest AIC scores for certain degree of freedom
```{r}
fit.step <- step(fit.null, 
                 scope=list(lower=fit.null, upper=fit.full), direction=  "both")
print(fit.step)
```
This gives us a logistic regression model which is a perfect fit for our data. The perfect fit with the given featurs is: 
glm(formula = Category ~ Total_Sugar + Sodium + Phosphorus + Total_Fat + Potassium, 
    family = binomial(link = "logit"), 
    data = Data_LRM)
    
No, fitting a model based on the results of the step function:
```{r}
fit_lrm <- glm(formula = Category ~ Total_Sugar + Sodium + Phosphorus + Total_Fat + Potassium,
               family = binomial(link = "logit"), 
               data = Data_LRM)
print(summary(fit_lrm))
```
This model gives a pretty good AIC score of 38 on 89 degrees of freedom. This has been achieved withought complicating the model.

According to our summary, everything apart from the intercept is significant in our fitted model.

Equation of the fitted model is:
log ( p(Vegetables)/p(Fruits) ) = (-0.687689 * Total_Sugar) + (0.069447 * Sodium) + (0.094477 * Phosphorus) - (5.064731 * Total_Fat) + (0.016426 * Potassium)

Note: fruits = 0 and Vegetables = 1

<h4> Model Analysis</h4>

We will find the <b>R2</b2> of the logistic Regression model fit above based on the function defined below:
```{r}
...
This shows that: Our model is a good fit to the data.

Now, we will test fit_lrm model's improvement over the null model using <b>chi2</b>
```{r}
...
3. So (1 - pchisq) is the prob of a test stastistic this good or better by chance 
...
p-value = 1 - pchisq(deviance, degrees of freedom). This indicates improved fit in this model over intercept only (null model) is significant as the hypothesis states that there is no significant differnce between the model that has been fit and the null model.

<b>Anova</b> Analysis of the model 
```{r}
anova_model <- fit.step$anova
print(anova_model)
```
The above results show that the AIC score keeps falling as and when every new feature is added to the model. Hence, coming to a very good AIC sore of 38.
